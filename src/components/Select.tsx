...
const getOptionValueDefault = <O,>(option: O) => (option as { value: unknown }).value as string;
const getOptionLabelDefault = <O,>(option: O) => (option as { label: string }).label;
const eq = <O,>(op1: SingleValue<O>, op2: SingleValue<O>) => getOptionValueDefault(op1) === getOptionValueDefault(op2);

const Select = <O = unknown, Multi extends boolean = boolean>(props: SelectProps<O, Multi>) => {
  ...
  const handleCloseDropdownOnBlur = (ev: FocusEvent) => {
    if (ev.relatedTarget && !containerRef.contains(ev.relatedTarget as HTMLElement)) {
      setOpen(false);
    }
  };
    ...
    <div ref={containerRef!} class="relative">
      ...
      >
        <Show when={!props.multi}>
          <Show when={props.value} fallback={<span>{value() ? getOptionLabelDefault(value()) : ''}</span>}>
            <span>{props.value ? getOptionLabelDefault(props.value) : ''}</span>
          </Show>
        </Show>
            ...
            <For each={value() ?? []}>
              {(v) => (
                <span class="flex cursor-default items-center gap-2 whitespace-nowrap rounded-md bg-primary-600 px-2 text-sm text-neutral-50">
                  ...
                  <button class="text-md font-bold hover:text-red-400" onClick={() => handleSelect(v)}>
                    x
                  </button>
                </span>
              )}
          ...
          <For each={props.options}>
            {(option, _) => (
              <li
                ...
                classList={{
                  'text-primary-500':
                    (!props.multi && value() && eq(value(), option)) ||
                    (props.multi && value() && value()?.some((el) => eq(el, option))),
                }}
